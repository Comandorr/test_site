...
def add_links():
    open()
    zapros = '''INSERT INTO quiz_content (quiz_id, question_id) VALUES (?, ?)'''
    cursor.execute(zapros, (0, 1)) # executemany -> execute
    conn.commit()
    cursor.execute(zapros, [0, 2])
    conn.commit()
    cursor.execute(zapros, [1, 3])
    conn.commit()
    cursor.execute(zapros, [1, 4])
    conn.commit()
    cursor.execute(zapros, [2, 5])
    conn.commit()
    cursor.execute(zapros, [2, 1])
    conn.commit()
    close()
...
def get_question(quiz_id, question_id):
    open()
    zapros = '''SELECT quiz_content.id, question.question, question.answer, question.wrong1, question.wrong2, question.wrong3
    FROM question, quiz_content
    WHERE quiz_content.question_id == question.id
    AND quiz_content.id > ? AND quiz_content.quiz_id == ?
    ''' # изменился текст запроса
    cursor.execute(zapros, [quiz_id, question_id])
